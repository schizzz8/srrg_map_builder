...
int main(int argc, char** argv) {
    ...
    cv::imshow("map",reference->traversabilityMap()->image());
    cv::waitKey(0);

    std::list<Serializable*> maps;
    maps.push_back(reference);
    cerr << "Writing: " << maps.size() << " objects" << endl;
    Serializer ser;
    ...
    for(std::list<Serializable*>::iterator it = maps.begin(); it != maps.end(); it++){
        Serializable* s = *it;
        ser.writeObject(*s);
    }
